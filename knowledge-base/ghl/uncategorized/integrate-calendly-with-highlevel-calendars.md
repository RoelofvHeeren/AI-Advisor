---
title: "Integrate Calendly with HighLevel Calendars"
...
## **Phone number sync (Location vs Form fields)**

  
HighLevel can import a guest’s phone number from Calendly in more than one way. When multiple sources are possible, you can choose what HighLevel should sync to the contact profile.

###   
**Automatic sync from meeting location phone number**

  
If the guest’s phone number is entered in the **Calendly meeting location** and there are **no phone fields** in the Calendly form, HighLevel automatically syncs that number to the contact.

###   
**Mapping modal when phone numbers come from Calendly form fields**

  
If a **phone number field exists** in the Calendly form (Q&A), HighLevel shows a mapping screen so you can choose what to sync:  
  

-   Select a phone value from a form field  
      
    
-   Select the meeting location phone field (if present)  
      
    
-   Choose **Do not sync** (to avoid mapping a phone number)  
      
    

Note: If the selected Calendly calendar/event type has **no phone fields** (neither location phone nor form phone), the phone sync step is skipped with no prompts or errors.

Shared calendars: If any user updates the phone number sync option, the change applies to all users linked to that calendar.
