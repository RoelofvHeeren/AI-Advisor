...
const KNOWLEDGE_BASE_DIR = path.join(process.cwd(), 'knowledge-base', 'ghl');
...
async function getEmbedding(text: string) {
    // Request 768 dimensions per genAI setup, slicing for Supabase pgvector schema (1536) if needed, 
    // but the DB natively expects what we feed it. We slice it to 1536.
    ...
    // Safety check just in case outputDimensionality is ignored
...
async function ingest() {
    console.log('Starting GoHighLevel Bulk Ingestion...');

    // 1. Create/Fetch GHL Advisor
    console.log('Creating/Fetching Advisor: GoHighLevel Support');
    ...
    if (!advisorId) {
        const { data: newAdvisor, error: createError } = await supabase
            .from('advisors')
            .insert({
                name: 'GoHighLevel Support',
                description: 'The complete official GoHighLevel Knowledge Base.',
                system_prompt: 'You are an expert GoHighLevel Support Advisor. You have access to every official support article. You answer questions directly, professionally, and accurately using only the knowledge base provided to you. If you provide a step-by-step guide, format it cleanly.',
                avatar_url: 'https://images.g2crowd.com/uploads/product/image/social_landscape/social_landscape_265d36e2f1e2f47306231bd7b6efcc99/gohighlevel.png'
            })
            .select()
            .single();
    ...
    console.log(`Found ${allFiles.length} Markdown articles.`);
    ...
    for (const filePath of allFiles) {
        ...
        const url = urlMatch ? urlMatch[1] : `ghl-kb-${title}`;
        ...
        const { data: doc, error: docError } = await supabase
            ...
            .insert({
                ...
                content: cleanText // Pre-formatted!
            })
            .select()
            .single();
        ...
        for (const chunkContent of chunks) {
            try {
                ...
                await sleep(150); // Be nice to Gemini API
            } catch (e: any) {
